...
- `react/renderer/components/LiteTextViewSpec/LiteTextShadowNode.{h,cpp}` — subclasses `ConcreteViewShadowNode` (reusing the generated `LiteTextViewComponentName`, named differently from the generated `LiteTextViewShadowNode` alias), sets `LeafYogaNode + MeasurableYogaNode` in `BaseTraits()`, and `measureContent` delegates to a `LiteTextMeasurementsManager`.
...
- `.../ComponentDescriptors.h` — **an override of the generated header of the same include path**, defining a measuring `LiteTextViewComponentDescriptor` (with an `adopt()` that wires the manager onto the shadow node).
- `LiteTextViewManager.kt` — overrides `measure(...)`: reads `text`/`fontSize` from the props `ReadableMap`, measures an off-screen `LiteTextView`, returns `YogaMeasureOutput.make(...)`.

Wiring gotchas (this is the fiddly part):
- **Registration is by include-shadowing, driven from `react-native.config.js`.** Autolinking generates `providerRegistry->add(concreteComponentDescriptorProvider<LiteTextViewComponentDescriptor>())` and `#include <react/renderer/components/LiteTextViewSpec/ComponentDescriptors.h>`. The root `react-native.config.js` sets the android `cmakeListsPath` to `src/main/jni/CMakeLists.txt` (replacing the default generated one). That CMakeLists compiles the generated codegen sources **plus** our custom sources into the `react_codegen_LiteTextViewSpec` target, and lists our jni dir on the include path **before** the generated dir — so `ComponentDescriptors.h` resolves to our measuring override. No app-side C++/OnLoad edits needed. `ComponentDescriptorProviderRegistry::add` is first-wins, but there's only one provider per handle here because the shadowed header is the only `LiteTextViewComponentDescriptor` the app ever sees.
...
- The `requestLayout()` re-measure hack in `LiteTextView.kt` is still needed: intrinsic sizing gives Yoga the size, but the mounted `TextView` still has to build its draw `Layout`.
