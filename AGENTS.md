...
- The `requestLayout()` re-measure hack in `RNPlainText.kt` is still needed: intrinsic sizing gives Yoga the size, but the mounted `TextView` still has to build its draw `Layout`.
